...
[Cmdlet(VerbsLifecycle.Disable, "MPDOutput", DefaultParameterSetName = "name")]
public class DisableMpdOutput: MpdCmdlet, IDisposable {
	[Parameter(
		Mandatory = true,
	Position = 0,
	...
	protected override void EndProcessing() {
		...
		this.mpc.DisableOutputs(this.selectedOutputs.Where(o => o.ID >= 0).Select(o => (uint)o.ID));
		foreach (var o in this.selectedOutputs) {
			WriteInformation($"Disabled output {o.Name}");
		}
		this.mpc.Dispose();
	}
}
